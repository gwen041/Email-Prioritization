...
const Logo: React.FC<LogoProps> = ({ className = '', size = 'md', showText = true }) => {
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <div 
        className="flex items-center justify-center bg-[#2E2996] rounded-lg shadow-sm"
      ...
      {showText && (
        <span className={`font-black tracking-tighter uppercase ${currentSize.font} text-[#1A1A1A]`}>
          Siftly
        </span>
      )}
    </div>
