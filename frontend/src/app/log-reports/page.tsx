...
export default function LogReports() {
    ...
    const stats = useMemo(() => {
        ...
        // Score Trend Distribution (Real-time active workload ONLY)
        const now = new Date();
        const trendBuckets = [
            { label: '>1mo', minHours: 720, maxHours: 8760, count: 0, avgScore: 0 },
            { label: '<1mo', minHours: 168, maxHours: 720, count: 0, avgScore: 0 },
            { label: '<1wk', minHours: 72, maxHours: 168, count: 0, avgScore: 0 },
            { label: '<3d', minHours: 24, maxHours: 72, count: 0, avgScore: 0 },
            { label: '<24h', minHours: 0, maxHours: 24, count: 0, avgScore: 0 },
        ];

        emails.forEach(e => {
            // EXCLUDE PAST DUE
            if (!e.deadline || e.urgency_label === 'Past Due') return;
            
            const deadlineDate = new Date(e.deadline);
            const diffHours = (deadlineDate.getTime() - now.getTime()) / (1000 * 60 * 60);
            
            if (diffHours < 0) return; // double check for future only

            for (let i = 0; i < trendBuckets.length; i++) {
                if (diffHours > trendBuckets[i].minHours && diffHours <= trendBuckets[i].maxHours) {
                    trendBuckets[i].count++;
                    trendBuckets[i].avgScore += (e.total_score || 0);
                    break;
                }
            }
        });

        const activeTotal = emails.filter(e => e.deadline && e.urgency_label !== 'Past Due').length || 1;
        const scoreTrend = trendBuckets.map(b => ({
            ...b,
            avgScore: b.count > 0 ? b.avgScore / b.count : 0,
            height: b.count > 0 ? Math.max(15, (b.count / activeTotal) * 100) : 5
        }));

        return {
            total, high, medium, low, pastDue, avgScore, compliance, sortedKeywords, scoreTrend,
            readRatios: {
        ...
        return (
            <div className="h-screen bg-[#F8F9FF] flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    <div className="w-12 h-12 border-4 border-[#2E2996] border-t-transparent rounded-full animate-spin"></div>
                    <p className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Analyzing Logs...</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <section className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 space-y-6">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="pt-2">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Urgency Breakdown</p>
                                    <div className="flex h-4 rounded-full overflow-hidden">
                                        <div className="bg-red-500" style={{ width: `${(stats?.high! / stats?.total!) * 100}%` }}></div>
                                        <div className="bg-yellow-400" style={{ width: `${(stats?.medium! / stats?.total!) * 100}%` }}></div>
                                        <div className="bg-emerald-400" style={{ width: `${(stats?.low! / stats?.total!) * 100}%` }}></div>
                                        <div className="bg-slate-300" style={{ width: `${(stats?.pastDue! / stats?.total!) * 100}%` }}></div>
                                    </div>
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="pt-2">
                                    <div className="flex items-center justify-between text-xs font-black uppercase tracking-widest text-slate-400 mb-2">
                                        ...
                                        <span className="text-slate-800">~12 mins</span>
                                    </div>
                                    <p className="text-[10px] text-slate-400 italic">Inferred from current interaction patterns</p>
                                </div>
                            </div>
                        </section>
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="pt-2 space-y-5">
                                    <div className="flex justify-between items-end">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Score Escalation Trend</p>
                                        <div className="text-[10px] font-black text-[#2E2996] uppercase bg-indigo-50 px-2 py-0.5 rounded">Active Workload Only</div>
                                    </div>
                                    <div className="relative h-24 bg-slate-50 rounded-2xl border border-slate-100 flex items-end px-4 gap-3 py-2">
                                        {stats?.scoreTrend.map((bucket, i) => (
                                            <div 
                                                key={i} 
                                                className="flex-1 bg-[#2E2996] rounded-t-sm opacity-30 hover:opacity-100 transition-all cursor-help group relative" 
                                                style={{ height: `${bucket.height}%` }}
                                            >
                                                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] font-bold px-3 py-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 shadow-lg">
                                                    {bucket.label}: {bucket.count} emails (Avg: {Math.round(bucket.avgScore)}%)
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    <div className="flex justify-between px-1">
                                        {stats?.scoreTrend.map((bucket, i) => (
                                            <span key={i} className="text-[9px] font-black uppercase text-slate-400">{bucket.label}</span>
                                        ))}
                                    </div>
                                    <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
                                        This chart visualizes your active workload. The bars show the volume of upcoming emails as their deadlines approach.
                                    </p>
                                </div>
                            </div>
                        </section>
